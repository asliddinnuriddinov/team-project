...
const Locations = () => {
    ...
    const handleSubmit = (event) => {
        ...
        if (isEditing) {
            fetch(`https://autoapi.dezinfeksiyatashkent.uz/api/locations/${data.id}`, {
                method: "PUT",
                headers: {
        ...
        <div style={{ background: '#E3EFFE' }} className='text-7xl h-full overflow-x-hidden container bg-white rounded-xl py-10'>
            <div className={`modal bg-white ${modal ? "inline-block" : "hidden"} drop-shadow-2xl w-[30%] h-[510px] pt-3 pl-8 pr-8 rounded-lg shadow-lg absolute m-auto top-0 bottom-0 right-0 left-0`}>
                <div className="flex items-center justify-between">
                    <h4 className="text-xl w-[100%]">{isEditing ? "Edit location" : "Add location"}</h4>
            ...
            <button className='m-10 ml-20 text-sm bg-blue-600 pt-3 pl-4 pr-4 pb-3 rounded-md text-white' onClick={handleAddClick}>Add location</button>
                    ...
                    {locations && locations.map((item, index) => (
                        <tr className='flex sm:items-center m-4' key={index}>
                            ...
                            <td className='flex gap-5 w-[200px] ml-10'>
                                ...
                                <button className='text-sm bg-red-600 pt-3 pl-4 pr-4 pb-3 rounded-md text-white' onClick={() => deleteLocation(item.id)}>Del</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
