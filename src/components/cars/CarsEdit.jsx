...
const CarsEdit = ({ isOpen, handleClose, id,reRenderTable,carData }) => {
    ...
    useEffect(()=>{
        setCategory(carData.category_id)
        setBrand(carData.brand_id)
        setModel(carData.model_id)
        setLocation(carData.location_id)
        setCity(carData.city_id)
        setInclusive(carData.inclusive)

        setValue('categoryAdd', carData.category_id);
        setValue('brandAdd', carData.brand_id);
        setValue('modelAdd', carData.model_id);
        setValue('lolcationAdd', carData.location_id);
        setValue('cityAdd', carData.city_id);
        setValue('color', carData.color);
        setValue('year', carData.year);
        setValue('seconds', carData.seconds);
        setValue('speed', carData.max_speed);
        setValue('people', carData.max_people);
        setValue('motor', carData.motor);
        setValue('transmission', carData.transmission);
        setValue('drive', carData.drive_side);
        setValue('fuel', carData.petrol);
        setValue('limit', carData.limitperday);
        setValue('deposit', carData.deposit);
        setValue('protection', carData.premium_protection);
        setValue('aed', carData.price_in_aed);
        setValue('usd', carData.price_in_usd);
        setValue('aed1', carData.price_in_aed_sale);
        setValue('usd1', carData.price_in_usd_sale);
        setValue('inclusive', carData.inclusive);
    },[carData])
    ...
    const {register,handleSubmit,formState:{isDirty,isSubmitting},setValue}=useForm()
  
    async function eeditCar({brandAdd,modelAdd,aed,aed1,usd,usd1,cityAdd,deposit,limit,fuel,motor,drive,transmission,color,year,seconds,categoryAdd,speed,people,protection,lolcationAdd,inclusive}){
      try{
        ...
        const response = await axios.put(url + `/cars/${id}`,formdata, {
          headers: {
        ...
        reRenderTable()
        toast.success("Changed Car successfully")
        handleClose()
      } 
      catch(err){   
      ...
      <form onSubmit={handleSubmit(eeditCar)} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col">
          ...
          <label className="inline-flex items-center cursor-pointer w-fit">
            <input {...register("inclusive")} type="checkbox" onChange={e=>setInclusive(e.target.checked)} checked={inclusive} className="sr-only peer"/>
